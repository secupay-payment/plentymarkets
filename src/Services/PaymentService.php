...
use Secupay\Helper\OrderItemSkuHelper;
...
class PaymentService
{
    ...
    public function createWebhook()
    {
        ...
        $this->sdkService->call('createWebhook', [
            ...
            'notificationUrl' => $webstoreConfig->domainSsl . '/secupay/update-transaction' . ($this->config->get('plenty.system.info.urlTrailingSlash', 0) == 2 ? '/' : '')
        ]);
    }
    ...
    public function executePayment(Order $order, PaymentMethod $paymentMethod): array
    {
        ...
        if (is_array($existingTransaction) && $existingTransaction['error']) {
            $this->getLogger(__METHOD__)->error('secupay::ExistingTransactionsError', $existingTransaction);
            return [
                'transactionId' => $transactionId,
        ...
        if (is_array($transaction) && $transaction['error']) {
            $this->getLogger(__METHOD__)->error('secupay::TransactionError', $transaction);
            return [
                'transactionId' => $transactionId,
                'type' => GetPaymentMethodContent::RETURN_TYPE_ERROR,
        ...
        if (is_array($paymentPageUrl) && isset($paymentPageUrl['error'])) {
            $this->getLogger(__METHOD__)->error('secupay::PaymentPageUrlError', $paymentPageUrl);
            return [
                'transactionId' => $transaction['id'],
                'type' => GetPaymentMethodContent::RETURN_TYPE_ERROR,
        ...
        foreach ($order->orderItems as $orderItem) {
            if (! empty($orderItem->orderProperties)) {
                ...
                foreach ($orderItem->orderProperties as $orderProperty) {
                    ...
                    $this->getLogger(__METHOD__)->debug('secupay::Variation', [
        ...
        foreach ($order->orderItems as $orderItem) {
            ...
            if (! array_key_exists($variationId, $itemIdsByVariationId)) {
                try {
                    ...
                    $this->getLogger(__METHOD__)->error('secupay::debug.basic', [
    ...
    private function getFailedUrl(): string
    {
        $lang = $this->session->getLocaleSettings()->language;
        $domain = $this->webstoreHelper->getCurrentWebstoreConfiguration()->domainSsl;
        return sprintf('%s/%s/secupay/fail-transaction', $domain, $lang);
    }
    ...
    {
        $status = $this->config->get('secupay.refund_successful_status');
        if (empty($status)) {
            return '11.2';
        } else {
            return $status;
    ...
    {
        $status = $this->config->get('secupay.refund_failed_status');
        if (empty($status)) {
            return '11.3';
        } else {
            return $status;
