...
class PaymentProcessController extends Controller
{
    ...
    public function failTransaction(Twig $twig, int $id)
    {
        ...
        $errorMessage = $this->frontendSession->getPlugin()->getValue('secupayPayErrorMessage');
        if ($errorMessage) {
            $this->notificationService->error($errorMessage);
            $this->frontendSession->getPlugin()->unsetKey('secupayPayErrorMessage');
        ...
        return $twig->render('secupay::Failure', [
            ...
            'payOrderFormUrl' => sprintf('/%s/secupay/pay-order/', $lang)
        ]);
    }
    ...
    public function payOrder(Request $request)
    {
        ...
        if ($result['type'] == GetPaymentMethodContent::RETURN_TYPE_REDIRECT_URL) {
            ...
            if (isset($result['content'])) {
                $this->frontendSession->getPlugin()->setValue('secupayPayErrorMessage', $result['content']);
            }
            // Construct the URL with the language
            $failUrl = sprintf('%s/secupay/fail-transaction/%s', $lang, $result['transactionId']);
    ...
    private function getPaymentMethodListForSwitch($paymentMethodId, $orderId)
    {
        ...
        foreach ($paymentMethods as $paymentMethod) {
            if ($paymentMethod->pluginKey == 'secupay') {
                $paymentMethodsForSwitch[] = [
                    'id' => $paymentMethod->id,
    ...
    private function checkOrderRetryStatus($statusId) {
        $orderRetryStatusString = $this->config->get('secupay.order_retry_status');
