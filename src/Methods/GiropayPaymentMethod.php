...
class GiropayPaymentMethod extends AbstractPaymentMethod
{
    ...
    {
        if ($this->configRepo->get('secupay.giropay_active') === "true") {
            return true;
        } else {
            return false;
        }
    ...
    public function getName(string $lang = 'de'): string
    {
        ...
        $title = $translator->trans('Secupay::Payment.GiropayTitle', [], $lang);
        if (! empty($title)) {
            return $title;
        } else {
            return 'Giropay';
    ...
    {
        $fee = $this->configRepo->get('secupay.giropay_fee');
        if (! empty($fee)) {
            return (float) $fee;
        } else {
            return 0.00;
    ...
    public function getDescription(string $lang = 'de'): string
    {
        ...
        $title = $translator->trans('Secupay::Payment.GiropayDescription', [], $lang);
        if (! empty($title)) {
            return $title;
        } else {
            return '';
    ...
    public function getIcon(string $lang = 'de'): string
    {
        ...
        $iconUrl = $translator->trans('Secupay::Payment.GiropayIconUrl', [], $lang);
        if (!empty($iconUrl)) {
            return $iconUrl;
        } else {
