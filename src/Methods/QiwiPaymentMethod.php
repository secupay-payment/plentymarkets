...
class QiwiPaymentMethod extends AbstractPaymentMethod
{
    ...
    {
        if ($this->configRepo->get('secupay.qiwi_active') === "true") {
            return true;
        } else {
            return false;
        }
    ...
    public function getName(string $lang = 'de'): string
    {
        ...
        $title = $translator->trans('secupay::Payment.QiwiTitle', [], $lang);
        if (! empty($title)) {
            return $title;
        } else {
            return 'QIWI';
    ...
    {
        $fee = $this->configRepo->get('secupay.qiwi_fee');
        if (! empty($fee)) {
            return (float) $fee;
        } else {
            return 0.00;
    ...
    public function getDescription(string $lang = 'de'): string
    {
        ...
        $title = $translator->trans('secupay::Payment.QiwiDescription', [], $lang);
        if (! empty($title)) {
            return $title;
        } else {
            return '';
    ...
    public function getIcon(string $lang = 'de'): string
    {
        ...
        $iconUrl = $translator->trans('secupay::Payment.QiwiIconUrl', [], $lang);
        if (!empty($iconUrl)) {
            return $iconUrl;
        } else {
