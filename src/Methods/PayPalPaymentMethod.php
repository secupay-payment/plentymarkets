...
class PayPalPaymentMethod extends AbstractPaymentMethod
{
    ...
    {
        if ($this->configRepo->get('secupay.paypal_active') === "true") {
            return true;
        } else {
            return false;
        }
    ...
    public function getName(string $lang = 'de'): string
    {
        ...
        $title = $translator->trans('Secupay::Payment.PayPalTitle', [], $lang);
        if (! empty($title)) {
            return $title;
        } else {
            return 'PayPal';
    ...
    {
        $fee = $this->configRepo->get('secupay.paypal_fee');
        if (! empty($fee)) {
            return (float) $fee;
        } else {
            return 0.00;
    ...
    public function getDescription(string $lang = 'de'): string
    {
        ...
        $title = $translator->trans('Secupay::Payment.PayPalDescription', [], $lang);
        if (! empty($title)) {
            return $title;
        } else {
            return '';
    ...
    public function getIcon(string $lang = 'de'): string
    {
        ...
        $iconUrl = $translator->trans('Secupay::Payment.PayPalIconUrl', [], $lang);
        if (!empty($iconUrl)) {
            return $iconUrl;
        } else {
