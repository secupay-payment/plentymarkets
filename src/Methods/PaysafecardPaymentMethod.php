...
class PaysafecardPaymentMethod extends AbstractPaymentMethod
{
    ...
    public function getName(string $lang = 'de'): string
    {
        ...
        $title = $translator->trans('secupay::Payment.PaysafecardTitle', [], $lang);
        if (! empty($title)) {
            return $title;
        } else {
            return 'paysafecard';
    ...
    {
        $fee = $this->configRepo->get('secupay.paysafecard_fee');
        if (! empty($fee)) {
            return (float) $fee;
        } else {
            return 0.00;
    ...
    public function getDescription(string $lang = 'de'): string
    {
        ...
        $title = $translator->trans('secupay::Payment.PaysafecardDescription', [], $lang);
        if (! empty($title)) {
            return $title;
        } else {
            return '';
    ...
    public function getIcon(string $lang = 'de'): string
    {
        ...
        $iconUrl = $translator->trans('secupay::Payment.PaysafecardIconUrl', [], $lang);
        if (!empty($iconUrl)) {
            return $iconUrl;
        } else {
