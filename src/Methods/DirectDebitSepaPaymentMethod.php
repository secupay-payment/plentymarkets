...
class DirectDebitSepaPaymentMethod extends AbstractPaymentMethod
{
    ...
    {
        if ($this->configRepo->get('secupay.directdebitsepa_active') == "true") {
            return true;
        } else {
            return false;
        }
    ...
    public function getName(string $lang = 'de'): string
    {
        ...
        $title = $translator->trans('secupay::Payment.DirectDebitSepaTitle', [], $lang);
        if (! empty($title)) {
            return $title;
        } else {
    ...
    {
        $fee = $this->configRepo->get('secupay.directdebitsepa_fee');
        if (! empty($fee)) {
            return (float) $fee;
        } else {
            return 0.00;
    ...
    public function getDescription(string $lang = 'de'): string
    {
        ...
        $title = $translator->trans('secupay::Payment.DirectDebitSepaDescription', [], $lang);
        if (! empty($title)) {
            return $title;
        } else {
            return '';
    ...
    public function getIcon(string $lang = 'de'): string
    {
        ...
        $iconUrl = $translator->trans('secupay::Payment.DirectDebitSepaIconUrl', [], $lang);
        if (!empty($iconUrl)) {
            return $iconUrl;
        } else {
