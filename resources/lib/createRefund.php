    ...
    if ($reductionAmount != $refundAmount) {
        ...
        foreach ($baseLineItems as $lineItem) {
            if ($lineItem->getQuantity() > 0) {
                $lineItemReductionTotal = SecupaySdkHelper::roundAmount($lineItem->getAmountIncludingTax() * $rate / $lineItem->getQuantity());
        ...
        $remainder = SecupaySdkHelper::roundAmount($refundAmount - $calculatedTotal);
        ...
        if (abs($remainder) >= 0.01 && count($fixedReductions) > 0) {
            $fixedReductions[0]->setUnitPriceReduction($fixedReductions[0]->getUnitPriceReduction() + SecupaySdkHelper::roundAmount($remainder));
        }
